...
object ALSSkill {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().master("local")
      ...
      .config("spark.mongodb.read.connection.uri", "mongodb://127.0.0.1/data.events")
      .config("spark.mongodb.write.connection.uri", "mongodb://127.0.0.1/data.events")
      .getOrCreate()
    val df = spark.read.format("mongodb").load()
    val skill = df.select(col("userid"), col("behavior"), explode(col("skill")).alias("skill"))
//    println(skill.show())
    val ratingScore = spark.createDataFrame(Seq(("apply", 5), ("search", 4), ("view", 3), ("ignore", 2), ("hated", 0))).toDF("behavior", "rating")
