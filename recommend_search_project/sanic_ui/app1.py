...
async def search_result(request):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post("http://localhost:9200/test_index_3/_search", data=jsont.dumps(query), headers={'Content-type': 'application/json'}) as res:
            data = await res.json()

    historical_items = list(mongo[MONGO_DATABASE].events.find({"userid": username}).sort("_id", -1).limit(10))
    user_model_recs = {}
    item_model_recs = {}
    content_base_weight = {"userid": "", "item": {}, "itemId": {}, "location": {}, "level": {}, "skill": {}}
    ...
    ranking = [{"score": doc["_score"] + user_model_recs.get(doc["_source"]["jobId"], 0)  + \
    ...
    return {"message": f"Results (10/{data['hits']['total']['value']}): ", "results":ranking, "username": username, "keyword": keyword}
...
async def recommend_items(request):
    ...
    matched_job = list(mongo[MONGO_DATABASE].events.aggregate([{"$match": {"userid": {"$in": same_view_user}}}, {"$group": {"_id": "$jobId", "view_count": {"$sum": 1}, "skill": {"$first": "$skill"}, "location": {"$first": "$location"}, "level": {"$first": "$level"}}}, {"$sort": {"view_count": -1}}]))
