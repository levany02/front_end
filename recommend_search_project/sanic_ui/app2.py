...
async def search_result(request):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post("http://localhost:9200/test_index_6/_search", data=jsont.dumps(query), headers={'Content-type': 'application/json'}) as res:
            data = await res.json()
